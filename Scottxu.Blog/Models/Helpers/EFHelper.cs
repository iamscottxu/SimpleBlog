...
namespace Scottxu.Blog.Models.Helper
{
    public class EFHelper
    {
        ...
        public IQueryable<T> Sort<T>(IQueryable<T> q, PageInfoViewModel pageInfo) =>
            q.SortBy(pageInfo.SortField + " " + pageInfo.SortDirection);

        // 排序
        public IQueryable<T> Sort<T>(IQueryable<T> q, string sortField, string sortDirection) =>
            q.SortBy(sortField + " " + sortDirection);

        public IQueryable<T> SortAndPage<T>(IQueryable<T> q, PageInfoViewModel pageInfo) => SortAndPage(q,
            pageInfo.PageIndex, pageInfo.PageSize, pageInfo.RecordCount, pageInfo.SortField, pageInfo.SortDirection);

        // 排序后分页
        public IQueryable<T> SortAndPage<T>(IQueryable<T> q, int pageIndex, int pageSize, int recordCount,
        ...
        {
            var t = DataBaseContext.Set<T>().Local.FirstOrDefault(x => x.Guid == keyGuid);
            if (t != null) return t;
        ...
        // 向现有实体集合中添加新项
        protected void AddEntities<T>(ICollection<T> existItems, IEnumerable<Guid> newItemGuids)
            where T : class, IKeyGuid, new()
        {
            foreach (var roleGuid in newItemGuids)
            {
                var t = Attach<T>(roleGuid);
                existItems.Add(t);
            }
        }
        ...
        protected void ReplaceEntities<T>(ICollection<T> existEntities, Guid[] newEntityGuids)
            where T : class, IKeyGuid, new()
        {
            ...
            {
                var tobeAdded = newEntityGuids.Except(existEntities.Select(x => x.Guid)).ToArray();
                var tobeRemoved = existEntities.Select(x => x.Guid).Except(newEntityGuids).ToArray();
